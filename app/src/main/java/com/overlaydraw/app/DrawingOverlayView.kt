 ...
 * - taperEnabled        : 끝과 시작을 가늘게 (날카로운 붓 느낌)
...
class DrawingOverlayView(context: Context) : View(context) {
    ...
    var taperEnabled: Boolean = false
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        ...
        when (event.actionMasked) {
            MotionEvent.ACTION_DOWN -> {
                val s = Stroke(penColor, penWidth, taperEnabled, eraserMode)
    ...
    private fun drawStroke(canvas: Canvas, s: Stroke) {
        ...
        // 테이퍼 선: 누적 길이를 따라 0→max→0 으로 굵기를 변화시켜
        // 짧은 선분 여러 개로 그린다. 양 끝이 뾰족해진다.
        ...
        for (i in 1 until n) {
            ...
            val w = (minW + (s.maxWidth - minW) * taperFactor).coerceAtLeast(minW)
